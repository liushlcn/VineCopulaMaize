...
if (runs) {
  ...
  ## filter records with at least five years and heading date recorders more than five years
  sts <- aoi_cale[, .(.N,
                      heading = sum(!is.na(heading)),
  ...
  ## get box plot information for different maize type over regions
...
# # 
# aoi_gcal %>%
#   melt.data.table(id.vars = c("id03", "zone"), measure.vars = c("heading", "maturity")) %>%
#   ggplot() +
#   geom_density(mapping = aes(value, color = zone)) +
#   facet_wrap(~variable) +
#   theme_basic()
# 
# lines(density(aoi_gcal$maturity - aoi_gcal$heading))
# plot(density(dt$maturity - dt$heading))
# 
# st_as_sf(aoi_gcal) %>%
#   ggplot() +
